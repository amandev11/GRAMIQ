...
class IcosahedronGeometry extends Geometry {
  constructor() {
    ...
    this.addVertex(-1, t, 0, 1, t, 0, -1, -t, 0, 1, -t, 0, 0, -1, t, 0, 1, t, 0, -1, -t, 0, 1, -t, t, 0, -1, t, 0, 1, -t, 0, -1, -t, 0, 1).addFace(
      ...
      10, 7, 6, 7, 1, 8, 3, 9, 4, 3, 4, 2, 3, 2, 6, 3, 6, 8, 3, 8, 9, 4, 9, 5, 2,
...
function createProgram(gl: WebGLRenderingContext, shaderSources: string[], attribLocations?: Record<string, number>) {
...
function makeBuffer(gl: WebGLRenderingContext, sizeOrData: number | ArrayBufferView, usage: number) {
...
class ArcballControl {
  ...
  update(deltaTime: number, targetFrameDuration = 16) {
    ...
    if (s > 0.000001) {
      rv = rad / (2 * Math.PI);
      this.rotationAxis = [this._combinedQuat[0] / s, this._combinedQuat[1] / s, this._combinedQuat[2] / s];
    }
...
class InfiniteGridMenu {
  ...
  #init(onInit: ((sk: InfiniteGridMenu) => void) | null) {
    ...
    this.discVAO = this.makeVertexArray([
  ...
  makeVertexArray(bufLocNumElmPairs: Array<[WebGLBuffer, number, number]>, indices?: Uint16Array) {
    const gl = this.gl;
  ...
  #initTexture() {
    const gl = this.gl;
    this.tex = this.createAndSetupTexture();
  ...
  #animate(deltaTime: number) {
    ...
    positions.forEach((p, ndx) => {
      ...
      const finalScale = s * this.scaleFactor;
  ...
  #render() {
    ...
    // Light-theme friendly clear so the sphere sits on the page, not in a void.
    gl.clearColor(0.973, 0.98, 0.992, 1);
...
export default function InfiniteMenu({ items = [], scale = 1 }: InfiniteMenuProps) {
  ...
  useEffect(() => {
    ...
    const handleActiveItem = (index: number) => {
      const itemIndex = index % Math.max(items.length, 1);
