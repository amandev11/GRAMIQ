...
const DEFAULT_INNER_GRADIENT = "linear-gradient(145deg,#134e4a8c 0%,#38bdf844 100%)";
...
const ProfileCardComponent = ({
  ...
  useEffect(() => {
    ...
    const handleClick = () => {
      ...
      const motionWithPermission = anyMotion as unknown as { requestPermission?: () => Promise<string> } | undefined;
      if (motionWithPermission && typeof motionWithPermission.requestPermission === "function") {
        motionWithPermission
          .requestPermission()
          .then((state: string) => {
            if (state === "granted") {
  ...
  const cardStyle = useMemo(
    () =>
      ({
        ...
        "--behind-glow-color": behindGlowColor ?? "rgba(45, 212, 191, 0.45)",
